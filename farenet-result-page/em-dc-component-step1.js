(function (w) {
  ...
  var s = document.createElement('script'); s.src = 'https://rawgit.com/nodeca/pako/master/dist/pako.js'; s.onload = saveResultPage; document.head.appendChild(s)
  function saveResultPage () {
    ...
    var content = [JSON.stringify({ a: 'F9', o: trip.id.substr(0, 3), d: trip.id.substr(4, 3), dd: info.departureDate, rd: info.returnDate, se: 'en-us', curc: 'USD' }), location.href, links.join('\n'), html].join('\n\n')
    w.fetch('https://ighvs9kf1a.execute-api.us-west-2.amazonaws.com/f9', { method: 'POST', body: pako.gzip(content) }).then((res) => res.json().then(body => { console.log(body) })).catch(console.error)
  }
})(window)
